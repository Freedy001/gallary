...
type TagEmbeddingRepository interface {
	...
	// 向量搜索（使用 pgvector）
	VectorSearchMainCategories(ctx context.Context, modelName string, embedding []float32, categoryIds []string) ([]TagEmbeddingWithDistance, error)
	VectorSearchByCategory(ctx context.Context, modelName string, categoryId string, embedding []float32, threshold float64, limit int) ([]TagEmbeddingWithDistance, error)
}
...
func (r *tagEmbeddingRepository) VectorSearchMainCategories(ctx context.Context, modelName string, embedding []float32, categoryIds []string) ([]TagEmbeddingWithDistance, error) {
	...
	err := database.GetDB(ctx).WithContext(ctx).
		...
		Where("tags.source_category_id IN ?", categoryIds).
		Where("tags.sub_category_id = ?", "__main_category__").
		Order("distance ASC").
		Preload("Tag").
		Find(&results).Error
	...
	for i, r := range results {
		...
		embeddings[i] = TagEmbeddingWithDistance{
			TagEmbedding: &e,
			Distance:     r.Distance,
			Similarity:   1 - r.Distance/2, // 将余弦距离(0-2)转换为相似度(0-1)
		}
	}
...
func (r *tagEmbeddingRepository) VectorSearchByCategory(ctx context.Context, modelName string, categoryId string, embedding []float32, threshold float64, limit int) ([]TagEmbeddingWithDistance, error) {
	vectorStr := model.FloatsToVectorString(embedding)
	// 将相似度阈值转换为距离阈值：distance = 2 * (1 - similarity)
	distanceThreshold := 2 * (1 - threshold)
	...
	err := database.GetDB(ctx).WithContext(ctx).
		...
		Joins("left join tags on tags.id = tag_embeddings.tag_id ").
		Where("model_name = ?", modelName).
		Where("tags.source_category_id = ?", categoryId).
		Where("tags.sub_category_id != ?", "__main_category__"). // 排除主分类向量
		Where("embedding <=> ? < ?", vectorStr, distanceThreshold).
		Order("distance ASC").
		Limit(limit).
		Preload("Tag").
		Find(&results).Error
