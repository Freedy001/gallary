    ...
    <Dialog as="div" class="relative z-50" @close="$emit('close')">
      ...
      <div class="fixed inset-0 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4">
          ...
          >
            <DialogPanel class="w-full max-w-2xl max-h-[90vh] transform overflow-hidden rounded-2xl bg-gray-900 border border-white/10 text-left align-middle shadow-xl transition-all flex flex-col">
              <div class="p-6 border-b border-white/10">
                <DialogTitle as="h3" class="text-lg font-medium leading-6 text-white flex items-center gap-2">
                  <ArrowsRightLeftIcon class="h-5 w-5 text-blue-400" />
                  创建存储迁移任务
                </DialogTitle>
              </div>
              ...
              <div class="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <div>
                      ...
                      <div class="relative">
                        <input
                            ...
                            class="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white transition-colors placeholder:text-gray-600 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                            placeholder="搜索或选择相册..."
                            type="text"
                        ...
                        <div v-if="selectedAlbums.length > 0" class="flex flex-wrap gap-2 mt-2">
                          <span
                              ...
                              :key="album.id"
                              class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-300 border border-blue-500/30"
                          >
                            {{ album.name }}
                            <button
                        ...
                        >
                          <button
                              ...
                              :key="album.id"
                              :class="form.filter?.album_ids?.includes(album.id) ? 'text-blue-300' : 'text-gray-300'"
                          ...
                          >
                            ...
                            <CheckIcon v-if="form.filter?.album_ids?.includes(album.id)" class="h-4 w-4 text-blue-400"/>
                          </button>
                        </div>
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-2">
                        <input
                            ...
                            class="flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                            type="date"
                        />
                ...
                <div v-if="preview" class="p-4 rounded-xl bg-white/5 border border-white/10">
                  <div class="text-sm font-medium text-white mb-2">预览</div>
                  <div class="flex items-center gap-6 text-xs text-gray-400">
                    <div>待迁移文件: <span class="text-white font-medium">{{ preview.files_count }} 个</span></div>
                    <div>文件大小: <span class="text-white font-medium">{{ formatFileSize(preview.total_size) }}</span></div>
                    <div>预计耗时: <span class="text-white font-medium">{{ formatEstimatedTime(preview.estimated_time_seconds) }}</span></div>
                  </div>
                </div>
              ...
              <div class="p-6 border-t border-white/10 flex gap-3">
                ...
                <button
                    ...
                    class="flex-1 inline-flex justify-center rounded-xl border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    type="button"
                    @click="handleSubmit"
                >
