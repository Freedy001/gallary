<template>
  <AppLayout>
    <template #header>
      <header class="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-white/5 bg-black/20 backdrop-blur-xl px-6 transition-all duration-300">
    ...
    <template #default>
      <div class="p-6 min-h-[calc(100vh-4rem)]">
        ...
        <div v-else class="grid gap-6 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
          ...
          >
            <!-- 顶部装饰光效 -->
            <div class="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"></div>

            <!-- 卡片头部 -->
            <div class="border-b border-white/5 p-5 bg-white/[0.02]">
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  <h3 class="font-medium text-gray-100 truncate text-lg tracking-tight" :title="share.title || '未命名分享'">
                    {{ share.title || '未命名分享' }}
                  </h3>
            ...
            <div class="p-5 flex-1 flex flex-col">
              <div class="space-y-3 text-sm text-gray-400 flex-1">
                ...
                <div class="flex items-center gap-2.5 text-xs">
                  ...
                  <span :class="isExpired(share) ? 'text-red-400/70' : 'text-gray-500'">{{ formatExpireDate(share.expire_at) }}</span>
                </div>
