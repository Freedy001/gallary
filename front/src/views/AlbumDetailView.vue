<template>
  <AppLayout>
    <template #header>
      <TopBar :upload-album-id="albumId">
        <!-- 自定义左侧：面包屑导航 -->
        <template #left>
    ...
    <template #default>
      <!-- 命令面板 -->
      <CommandPalette />
      <ImageGrid mode="gallery" :exclude-album-id="albumId" />
    </template>
  </AppLayout>
</template>
...
async function handleRemoveFromAlbum() {
  ...
  try {
    await albumApi.removeImages(albumId.value, ids)
    // 从列表移除
...
onMounted(async () => {
  ...
  await imageStore.refreshImages(async (page: number, size: number): Promise<Pageable<Image>> => {
    return (await albumApi.getImages(albumId.value, page, size)).data
  }, pageSize)
})
